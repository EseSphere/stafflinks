...
$clientName = trim(($row['team_first_name'] ?? '') . ' ' . ($row['team_last_name'] ?? ''));
$_SESSION['clientId'] = $row['uryyTteamoeSS4'] ?? '';
$stmt->close();
?>
        ...
        async function loadVisits(date) {
            try {
                const response = await fetch(
                    `get_team_visits.php?date=${encodeURIComponent(date)}&clientId=${encodeURIComponent(clientId)}`
                );
