...
if (isset($_POST['btnStatusPermanent'])) {
    ...
    $txtReason = $_POST['txtReasonForPermanent'] ?? '';
    $txtTimeForPermanent = $_POST['txtTimeForPermanent'] ?? '';
    $txtNoteForPermanent = $_POST['txtNoteForPermanent'] ?? '';
    $txtStartDate = $_POST['txtStartDate'] ?? '';
    $txtEndDate = 'TFN'; // Permanent statuses are open-ended

    $statusColors = [
        'Active'      => '',
        'Deceased'    => 'rgba(231, 76, 60,1.0)',
        'LeftService' => 'rgba(142, 68, 173,1.0)',
        'Permanent'   => 'rgba(52, 31, 151,1.0)',
    ...
    try {
        // Check for existing permanent/open-ended status
        $checkSql = "SELECT * FROM tbl_client_status_records WHERE col_client_Id = ? 
        AND (col_end_date = 'TFN' OR col_end_date >= CURDATE() OR col_end_date = 'Active') ORDER BY userId DESC LIMIT 1";
        ...
        if ($result->num_rows > 0) {
            ...
            if ($txtReason === 'Active') {
                ...
                // Reset future schedules to Scheduled
                $scheduleSql = "UPDATE tbl_schedule_calls SET call_status = 'Scheduled' WHERE uryyToeSS4 = ? AND Clientshift_Date >= ?";
                ...
                if ($varStartDate == $txtStartDate) {
                    ...
                    $updateStmt = $conn->prepare($updateSql);
                    $updateStmt->bind_param("ssssssi", $txtReason, $txtTimeForPermanent, $txtNoteForPermanent, $colStatusColor, $txtStartDate, $txtEndDate, $row['userId']);
                    $updateStmt->execute();
                    $updateStmt->close();

                    // Cancel all future schedules for this client
                    $scheduleSql = "UPDATE tbl_schedule_calls SET call_status = 'Cancelled' WHERE uryyToeSS4 = ? AND Clientshift_Date >= ? AND dateTime_in >= ?";
                    $scheduleStmt = $conn->prepare($scheduleSql);
                    $scheduleStmt->bind_param("sss", $txtClientId, $txtStartDate, $txtTimeForPermanent);
                    $scheduleStmt->execute();
                    $scheduleStmt->close();
                    ...
                    $insertStmt->bind_param("ssssssssss", $txtClientName, $txtReason, $txtTimeForPermanent, $txtNoteForPermanent, $colStatusColor, $txtStartDate, $txtEndDate, $txtClientCity, $txtClientId, $txtCompanyId);
                    $insertStmt->execute();
                    $insertStmt->close();
