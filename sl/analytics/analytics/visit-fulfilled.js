const dbName = 'VisitFullfiledDB';
const storeName = 'VisitFullfiledResults';
...
document.getElementById('trendForm').addEventListener('submit', async function (e) {
    ...
    try {
        const response = await fetch('processing-visit-fullfiled.php', {
            method: 'POST',
...
function renderChart(labels, counts) {
    ...
    const canvas = document.getElementById('barChartVisitFullfiled');
    if (!canvas) {
        console.error('Canvas element with ID "barChartVisitFullfiled" not found.');
        return;
    }
    ...
    chartInstance = new Chart(ctx, {
        ...
        data: {
            labels: labels,
            datasets: [{
                label: 'Visit fullfiled per period',
                data: counts,
