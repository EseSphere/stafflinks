...
try {
    // Chart data
    $stmt = $conn->prepare("SELECT COUNT(*) FROM `tbl_clients_task_records` WHERE `dateTime` BETWEEN ? AND ?");
    ...
    $totalQuery = $conn->prepare("SELECT COUNT(*) as total FROM `tbl_clients_task_records` WHERE `dateTime` BETWEEN ? AND ?");
    ...
    $recordsQuery = $conn->prepare("SELECT `client_taskName`, COUNT(*) as count FROM `tbl_clients_task_records` WHERE `dateTime` BETWEEN ? AND ? GROUP BY `client_taskName` ORDER BY count DESC");
    $recordsQuery->bind_param("ss", $rangeStart, $rangeEnd);
    $recordsQuery->execute();
    $result = $recordsQuery->get_result();

    while ($row = $result->fetch_assoc()) {
        $name = htmlspecialchars($row['client_taskName']);
        $count = (int)$row['count'];
