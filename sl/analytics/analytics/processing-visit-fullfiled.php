...
try {
    // Chart data
    $stmt = $conn->prepare("SELECT COUNT(*) FROM `tbl_daily_shift_records` WHERE `shift_date` BETWEEN ? AND ?");
    ...
    $totalQuery = $conn->prepare("SELECT COUNT(*) as total FROM `tbl_daily_shift_records` WHERE `shift_date` BETWEEN ? AND ?");
    ...
    $recordsQuery = $conn->prepare("SELECT `client_name`, COUNT(*) as count FROM `tbl_daily_shift_records` WHERE `shift_date` BETWEEN ? AND ? GROUP BY `client_name` ORDER BY count DESC");
