const dbName = 'ScheduledVisitDB';
const storeName = 'ScheduledVisitResults';
...
document.getElementById('trendForm').addEventListener('submit', async function (e) {
    ...
    try {
        const response = await fetch('processing-scheduled-visit.php', {
            method: 'POST',
...
function renderChart(labels, counts) {
    ...
    const canvas = document.getElementById('barChartSchVisit');
    if (!canvas) {
        console.error('Canvas element with ID "barChartSchVisit" not found.');
        return;
    }
