...
<script>
    ...
    async function loadMMH() {
        const uryyToeSS4 = getQueryParam('uryyToeSS4');
        if (!uryyToeSS4) {
            document.getElementById('mmhContent').textContent = 'No client selected.';
            return;
        }
        ...
        req.onsuccess = e => {
            ...
            if (!client) {
                document.getElementById('mmhContent').textContent = 'Client not found.';
                return;
            }
            ...
            // My Medical History content
            const content = client.my_medical_history || 'No data available.';
            document.getElementById('mmhContent').innerHTML = content.split(/\n\s*\n/).map(p => `<p>${p.trim().replace(/\n/g,'<br>')}</p>`).join('');
