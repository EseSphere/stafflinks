    ...
    <div class="row gutters-sm">
        ...
        <div class="col-md-12 mb-3">
            <div class="card p-3 assessment-card">
                <h5>My Physical Health</h5>
                <hr>
                <div id="mphContent">Loading...</div>
            </div>
        </div>
...
<script>
    ...
    async function loadMPH() {
        const uryyToeSS4 = getQueryParam('uryyToeSS4');
        if (!uryyToeSS4) {
            document.getElementById('mphContent').textContent = 'No client selected.';
            return;
        }
        ...
        req.onsuccess = e => {
            ...
            if (!client) {
                document.getElementById('mphContent').textContent = 'Client not found.';
                return;
            }
            ...
            // My Physical Health content
            const content = client.my_physical_health || 'No data available.';
            document.getElementById('mphContent').innerHTML = content.split(/\n\s*\n/).map(p => `<p>${p.trim().replace(/\n/g,'<br>')}</p>`).join('');
