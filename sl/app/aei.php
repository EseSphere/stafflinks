    ...
    <div class="row gutters-sm">
        ...
        <div class="col-md-12 mb-3">
            <div class="card p-3 assessment-card">
                <h5>Assistive Equipment I Use</h5>
                <hr>
                <div id="aeiContent">Loading...</div>
            </div>
        </div>

        <!-- Navigation to Other Assessments -->
...
<script>
    ...
    async function loadAEI() {
        const uryyToeSS4 = getQueryParam('uryyToeSS4');
        if (!uryyToeSS4) {
            document.getElementById('aeiContent').textContent = 'No client selected.';
            return;
        }
        ...
        req.onsuccess = e => {
            ...
            if (!client) {
                document.getElementById('aeiContent').textContent = 'Client not found.';
                return;
            }
            ...
            // Assistive Equipment content
            const content = client.assistive_equipment_i_use || 'No data available.';
            document.getElementById('aeiContent').innerHTML = content.split(/\n\s*\n/).map(p => `<p>${p.trim().replace(/\n/g,'<br>')}</p>`).join('');
            ...
            const assessments = [{
                ...
                {
                    title: 'My Likes & Dislikes',
                    link: `mlad.php?uryyToeSS4=${client.uryyToeSS4}`,
                    icon: 'bi-emoji-smile'
                },
                {
                ...
                },
                {
                    ...
                    link: `mmh_mental.php?uryyToeSS4=${client.uryyToeSS4}`,
                    icon: 'bi-brain'
                },
                {
