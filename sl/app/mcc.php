    ...
    <div class="row gutters-sm">
        ...
        <div class="col-md-12 mb-3">
            <div class="card p-3 assessment-card">
                <h5>My Current Condition</h5>
                <hr>
                <div id="mccContent">Loading...</div>
            </div>
        </div>
...
<script>
    ...
    async function loadMCC() {
        ...
        if (!uryyToeSS4) {
            document.getElementById('mccContent').textContent = 'No client selected.';
            return;
        }
        ...
        req.onsuccess = e => {
            ...
            if (!client) {
                document.getElementById('mccContent').textContent = 'Client not found.';
                return;
            }
            ...
            const content = client.my_current_condition || 'No data available.';
            document.getElementById('mccContent').innerHTML = content.split(/\n\s*\n/).map(p => `<p>${p.trim().replace(/\n/g,'<br>')}</p>`).join('');
