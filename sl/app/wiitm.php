...
<div class="main-wrapper container mt-3">
    <div class="row gutters-sm">
        ...
        <!-- What is Important to Me Card -->
        <div class="col-md-12 mb-3">
            <div class="card p-3 assessment-card">
                <h5>What is Important to Me</h5>
                <hr>
                <div id="wiitmContent">Loading...</div>
            </div>
        </div>
...
<script>
    ...
    async function loadWIITM() {
        const uryyToeSS4 = getQueryParam('uryyToeSS4');
        if (!uryyToeSS4) {
            document.getElementById('wiitmContent').textContent = 'No client selected.';
            return;
        }
        ...
        req.onsuccess = e => {
            ...
            if (!client) {
                document.getElementById('wiitmContent').textContent = 'Client not found.';
                return;
            }
            ...
            const content = client.what_is_important_to_me || 'No data available.';
            const formattedContent = content.split(/\n\s*\n/).map(p => `<p>${p.trim().replace(/\n/g,'<br>')}</p>`).join('');
            document.getElementById('wiitmContent').innerHTML = formattedContent;
