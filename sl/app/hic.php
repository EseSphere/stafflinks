    ...
    <div class="row gutters-sm">
        ...
        <div class="col-md-12 mb-3">
            <div class="card p-3 assessment-card">
                <h5>How I Communicate</h5>
                <hr>
                <div id="hicContent">Loading...</div>
            </div>
        </div>
...
<script>
    ...
    async function loadHIC() {
        const uryyToeSS4 = getQueryParam('uryyToeSS4');
        if (!uryyToeSS4) {
            document.getElementById('hicContent').textContent = 'No client selected.';
            return;
        }
        ...
        req.onsuccess = e => {
            ...
            if (!client) {
                document.getElementById('hicContent').textContent = 'Client not found.';
                return;
            }
            ...
            // How I Communicate content
            const content = client.how_i_communicate || 'No data available.';
            document.getElementById('hicContent').innerHTML = content.split(/\n\s*\n/).map(p => `<p>${p.trim().replace(/\n/g,'<br>')}</p>`).join('');
