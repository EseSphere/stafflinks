...
$sql = "SELECT 
        ...
        WHERE (Clientshift_Date = ? AND uryyToeSS4 = ? AND call_status = ?)
        ORDER BY 
            FIELD(care_calls, 'Morning', 'Extra Morning', 'Lunch', 'Extra Lunch', 'Tea', 'Extra Tea', 'Bed', 'Extra Bed')";

$callStatus = 'Completed'; // Assuming you want to filter by completed visits
$stmt = $conn->prepare($sql);
if ($stmt) {
    $stmt->bind_param("sss", $date, $clientId, $callStatus);
    $stmt->execute();
