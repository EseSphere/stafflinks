    ...
    <div class="pcoded-content">
                ...
                <div class="row align-items-center">
                    <div class="col-md-12">
                        <div class="page-header-title">
                            <h4 class="m-b-10">Invoice dashboard</h4>
                            <p>Select a date range and contract and generate an invoice</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="row">
                        ...
                        <div class="row">
                            ...
                            <div style="margin-top:15px;" class="col-md-12 col-xl-12">
                                ...
                                <select name="txtClientContract" class="form-control" id="exampleFormControlSelect1">
                                    ...
                                    <?php
                                    $stmt = $conn->prepare("SELECT * FROM tbl_invoice_rate WHERE col_company_id = ? ORDER BY userId ASC");
                                    $stmt->bind_param("s", $_SESSION['usr_compId']);
                                    $stmt->execute();
                                    $getContract = $stmt->get_result();
                                    while ($fetchData = $getContract->fetch_array()) {
                                        echo "
                            ...
                            <div style="margin-top:25px;" class="col-md-12 col-xl-12">
                                ...
                                <select name="txtCareRecipient" class="form-control" id="exampleFormControlSelect1">
                                    ...
                                    <?php
                                    $stmt = $conn->prepare("SELECT * FROM tbl_daily_shift_records WHERE shift_date >= ? AND shift_date <= ? AND col_company_id = ? GROUP BY client_name ASC");
                                    $stmt->bind_param("ssi", $txtStartDate, $txtEndDate, $_SESSION['usr_compId']);
                                    $stmt->execute();
                                    $getCareRecipient = $stmt->get_result();
                                    while ($fetchData = $getCareRecipient->fetch_array()) {
                                        echo "
            ...
            <div class="col-xl-9 col-md-9">
                    ...
                    <div class="col-xl-4 col-md-4">
                        <div class="card flat-card widget-primary-card">
                            <div class="row-table">
                                ...
                                <div class="col-sm-9">
                                    <h4><?php echo '£' . $total_invoice_payment; ?></h4>
                                    <p style="font-size:16px; font-weight:600;">Balance</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-xl-4 col-md-4">
                        <div class="card flat-card widget-purple-card">
                            <div class="row-table">
                                ...
                                <div class="col-sm-9">
                                    <h4><?php echo $formatted_invoice_time; ?></h4>
                                    <p style="font-size:16px; font-weight:600;">Hour</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <form action="./processing-restore-visits" method="post" id="RestorVisForm" enctype="multipart/form-data" autocomplete="off"></form>
                <input form="RestorVisForm" type="hidden" name="txtCompanyId" value="<?php print $_SESSION['usr_compId']; ?>">
                <input form="RestorVisForm" type="hidden" name="txtPeriodStart" value="<?php print $txtStartDate; ?>">
                <input form="RestorVisForm" type="hidden" name="txtPeriodEnd" value="<?php print $txtEndDate; ?>">
                <input form="RestorVisForm" type="hidden" name="txtCareGiverId" value="<?php print $txtCareGiver; ?>">
                        ...
                        <div style="padding: 0;" class="col-md-4 col-xl-4">
                            <div class="d-flex justify-content-end">
                                <div class="btn-group" role="group" aria-label="Basic example">
                                    <button form="RestorVisForm" type="submit" name="btnRestoreVisits" style="width: 130px;" class="btn btn-info">
                                        Restore visits
                                    </button>
                                    <button form="InvoiceForm" type="submit" name="btnGenerateInvoice" class="btn btn-secondary btn-small">
                                        Generate invoice
                                    </button>
                                </div>
                            </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped table-hover mb-0">
                            ...
                            <tbody>
                                ...
                                AND (uryyToeSS4 = '$txtCareRecipient' OR shift_status = '$txtCareRecipient') AND col_visit_confirmation = 'Confirmed' 
                                ...
                                AND `col_company_Id` = '" . $_SESSION['usr_compId'] . "') GROUP BY uryyToeSS4 ORDER BY client_name LIMIT $offset, $total_records_per_page");
                                while ($row = mysqli_fetch_array($result)) {
                                    ...
                                    WHERE ((uryyToeSS4 = '$client_specialId' OR shift_status = 'Checked in') AND col_visit_confirmation = 'Confirmed' 
                                    AND `col_company_Id` = '" . $_SESSION['usr_compId'] . "') ");
                                    ...
                                    AS total_worked_hours FROM `tbl_daily_shift_records` WHERE ((uryyToeSS4 = '$client_specialId' OR shift_status = 'Checked in') AND col_visit_confirmation = 'Confirmed' 
                                    AND `col_company_Id` = '" . $_SESSION['usr_compId'] . "') ");
                                    ...
                                    $sql_total_mileage = mysqli_query($conn, "SELECT SUM(`col_mileage`) AS total_mileage 
                                    FROM `tbl_daily_shift_records` WHERE (uryyToeSS4 = '$client_specialId' 
                                    AND `col_company_Id` = '" . $_SESSION['usr_compId'] . "') ");
