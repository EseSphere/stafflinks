...
if (isset($_POST['txtDate'])) {
    ...
    header("Location: ./schedulled-visits?uryyToeSS4=$txtClientId");
}
...
$stmt->close();
$stmt = $conn->prepare("SELECT COUNT(*) FROM tbl_schedule_calls WHERE uryyToeSS4 = ? AND call_status = 'Scheduled' AND col_company_Id = ?");
$stmt->bind_param("ss", $uryyToeSS4, $_SESSION['usr_compId']);
$stmt->execute();
    ...
    <div class="pcoded-content">
        <div class="row">
            <div class="col-md-5 col-xl-5 col-sm-5">
                <h4>Scheduled visits <?php echo " (" . $rowcount . ")"; ?></h4>
            ...
            <div class="col-md-12">
                <div id="yourdiv">
                    ...
                    <div class="row">
                        ...
                        $visitDate = $visitCookieDate;
                        $clientId = $uryyToeSS4;

                        $stmt = $conn->prepare("SELECT first_carer_Id, Clientshift_Date FROM tbl_schedule_calls 
                        WHERE Clientshift_Date = ? AND call_status = 'Scheduled' AND uryyToeSS4 = ? AND col_company_Id = ? 
                        ORDER BY userId ASC LIMIT 1");
                        $stmt->bind_param("ssi", $visitDate, $clientId, $companyId);
                        $stmt->execute();
                        $stmt->bind_result($varRecentCarer, $varCareCallDate);
                        $stmt->fetch();
                        $stmt->close();

                        echo "<h5><strong><i class='feather mr-2 icon-calendar'></i> " . date('l', strtotime($visitDate)) . "</strong></h5>";

                        $query = "
                        SELECT c.userId, c.dateTime_in, c.dateTime_out, c.first_carer, c.first_carer_Id, c.bgChange, c.call_status, c.Clientshift_Date 
                        FROM tbl_schedule_calls c
                        LEFT JOIN tbl_cancelled_call a ON c.uryyToeSS4 = a.col_client_Id AND c.care_calls = a.col_care_call AND CURDATE() = a.col_date
                        WHERE c.first_carer_Id = ? AND c.Clientshift_Date = ? AND c.call_status = 'Scheduled' AND a.col_client_Id IS NULL AND c.uryyToeSS4 = ? AND c.col_company_Id = ? ORDER BY c.dateTime_in ASC";

                        $stmt = $conn->prepare($query);
                        $stmt->bind_param("issi", $varRecentCarer, $visitDate, $clientId, $companyId);
                        $stmt->execute();
                        $result = $stmt->get_result();
                                            ...
                                            <div class='card-body p-0'>
                                                <div style='width:100%; height:auto; padding:8px 0px 8px 22px; font-weight:600; font-size:16px;'>
                                                    ...
                                                    <table style='border:none;'>
                                                        ...
                                                        <tr>
                                                            <td><span style='font-weight:600;'><i class='feather mr-2 icon-clock'></i> {$row['dateTime_in']} </span></td>
                                                            <td style='padding: 0px 0px 0px 35px;'><span style='font-weight:600;'><i class='feather mr-2 icon-clock'></i> {$row['dateTime_out']} </span></td>
                                                        </tr>
                                                    </table>   
                                                </div>
                                                <div style='width:100%; height:auto; padding:8px 0px 8px 22px; font-weight:600; font-size:16px;'>
                                                    <span style='font-weight:22px;'><i class='feather mr-2 icon-users'></i> {$row['first_carer']} </span>
                                                </div>
                                                <div style='width: 100%; font-weight:600; height:auto; padding:8px; background-color: {$row['bgChange']}; color:#fff;'>
                                                    <span style='font-weight:600;'><i class='feather mr-2 icon-check-square'></i> {$row['call_status']} </span>
