...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        async function loadMonthlyVisits(date) {
            try {
                const month = date.getMonth() + 1; // JS months are 0-indexed
                const year = date.getFullYear();
                const response = await fetch(
                    `get_monthly_visits.php?year=${year}&month=${month}&clientId=${encodeURIComponent(clientId)}`
                );
                ...
                for (let i = 1; i <= daysInMonth; i++) {
                    ...
                    const dayStr = formatDateValue(day);
                    ...
                    dayTitle.textContent = `${dayName} (${formatDateDisplay(day)})`;
                    ...
                    <thead class="table-light">
                        <tr>
                            <th>Morning</th><th class="extra-col extra-morning">Extra Morning</th>
                            <th>Lunch</th><th class="extra-col extra-lunch">Extra Lunch</th>
                            <th>Tea</th><th class="extra-col extra-tea">Extra Tea</th>
                            <th>Bed</th><th class="extra-col extra-bed">Extra Bed</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <td class="Morning"></td><td class="Extra Morning extra-col extra-morning"></td>
                            <td class="Lunch"></td><td class="Extra Lunch extra-col extra-lunch"></td>
                            <td class="Tea"></td><td class="Extra Tea extra-col extra-tea"></td>
                            <td class="Bed"></td><td class="Extra Bed extra-col extra-bed"></td>
                        </tr>
                    </tbody>
                `;
                        ...
                        visits.forEach((visit, index) => {
                            ...
                            div.addEventListener('click', () => {
                                window.location.href =
                                    `visit-details.php?userId=${visit.visit_id}&spec=${visit.client_specId}`;
                            });
                ...
                if (!hasAnyVisits) visitsBox.innerHTML = '<p>No visits available for this month.</p>';
        ...
        printBtn.addEventListener('click', () => {
            const container = document.querySelector('.container-fluid').cloneNode(true);
            ...
            container.querySelectorAll('table').forEach(table => {
                ...
                for (let i = headerCells.length - 1; i >= 0; i--) {
                    if (!bodyRow.children[i].querySelector('.visit-box')) {
                        bodyRow.children[i].remove();
                        headerCells[i].remove();
                    }
                }
                if (table.querySelectorAll('tbody td').length === 0) table.parentElement.remove();
            });

            if (!container.querySelector('table')) container.innerHTML =
                '<p>No visits available for this month.</p>';
            container.querySelectorAll('div.mb-4').forEach(div => div.classList.add('day-container'));
            ...
            @media print { body { margin:0;padding:0;width:100%; } table{width:100%;border-collapse:collapse;} th,td{padding:8px;border:1px solid #ddd;text-align:left;} .day-container{page-break-inside:avoid;margin-bottom:20px;} .day-container:nth-child(2n){page-break-after:always;} button,a,input{display:none !important;} }
