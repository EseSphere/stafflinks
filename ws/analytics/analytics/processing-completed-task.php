...
try {
    // Chart data
    $stmt = $conn->prepare("SELECT COUNT(*) FROM `tbl_finished_tasks` WHERE `dateTime` BETWEEN ? AND ?");
    ...
    $totalQuery = $conn->prepare("SELECT COUNT(*) as total FROM `tbl_finished_tasks` WHERE `dateTime` BETWEEN ? AND ?");
    ...
    $recordsQuery = $conn->prepare("SELECT `task_name`, COUNT(*) as count FROM `tbl_finished_tasks` WHERE `dateTime` BETWEEN ? AND ? GROUP BY `task_name` ORDER BY count DESC");
