...
document.getElementById('trendForm').addEventListener('submit', async function (e) {
    ...
    try {
        const response = await fetch('processing-care-plan.php', {
            method: 'POST',
...
function renderChart(labels, counts) {
    ...
    const canvas = document.getElementById('barChartCarePlan');
    if (!canvas) {
        console.error('Canvas element with ID "barChartCarePlan" not found.');
        return;
    }
