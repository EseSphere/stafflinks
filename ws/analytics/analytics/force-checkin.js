const dbName = 'ForceCheckInDB';
const storeName = 'ForceCheckInResults';
...
document.getElementById('trendForm').addEventListener('submit', async function (e) {
    ...
    try {
        const response = await fetch('processing-force-checkin.php', {
            method: 'POST',
...
function renderChart(labels, counts) {
    ...
    const canvas = document.getElementById('barChartForceCheckIn');
    if (!canvas) {
        console.error('Canvas element with ID "barChartForceCheckIn" not found.');
        return;
    }
    ...
    chartInstance = new Chart(ctx, {
        ...
        data: {
            labels: labels,
            datasets: [{
                label: 'Force Check In per period',
                data: counts,
                backgroundColor: 'rgba(64, 115, 158,1.0)',
                borderColor: 'rgba(220, 221, 225,1.0)',
                borderWidth: 1
            }]
        },
        options: {
