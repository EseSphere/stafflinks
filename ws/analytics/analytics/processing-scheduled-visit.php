...
try {
    // Chart data
    $stmt = $conn->prepare("SELECT COUNT(*) FROM `tbl_schedule_calls` WHERE `Clientshift_Date` BETWEEN ? AND ?");
    ...
    $totalQuery = $conn->prepare("SELECT COUNT(*) as total FROM `tbl_schedule_calls` WHERE `Clientshift_Date` BETWEEN ? AND ?");
    ...
    $recordsQuery = $conn->prepare("SELECT `client_name`, COUNT(*) as count FROM `tbl_schedule_calls` WHERE `Clientshift_Date` BETWEEN ? AND ? GROUP BY `client_name` ORDER BY count DESC");
