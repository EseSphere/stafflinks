...
try {
    // Chart data
    $stmt = $conn->prepare("SELECT COUNT(*) FROM `tbl_clients_medication_records` WHERE `dateTime` BETWEEN ? AND ?");
    ...
    $totalQuery = $conn->prepare("SELECT COUNT(*) as total FROM `tbl_clients_medication_records` WHERE `dateTime` BETWEEN ? AND ?");
    ...
    $recordsQuery = $conn->prepare("SELECT `med_name`, COUNT(*) as count FROM `tbl_clients_medication_records` WHERE `dateTime` BETWEEN ? AND ? GROUP BY `med_name` ORDER BY count DESC");
    $recordsQuery->bind_param("ss", $rangeStart, $rangeEnd);
    $recordsQuery->execute();
    $result = $recordsQuery->get_result();

    while ($row = $result->fetch_assoc()) {
        $name = htmlspecialchars($row['med_name']);
        $count = (int)$row['count'];
