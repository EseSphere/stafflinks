const dbName = 'CompletedMedicationDB';
const storeName = 'CompletedMedicationResults';
...
document.getElementById('trendForm').addEventListener('submit', async function (e) {
    ...
    try {
        const response = await fetch('processing-completed-medication.php', {
            method: 'POST',
...
function renderChart(labels, counts) {
    ...
    const canvas = document.getElementById('barChartCompMedication');
    if (!canvas) {
        console.error('Canvas element with ID "barChartCompMedication" not found.');
        return;
    }
    ...
    chartInstance = new Chart(ctx, {
        ...
        data: {
            labels: labels,
            datasets: [{
                label: '% of completed medications',
                data: counts,
                backgroundColor: 'rgba(111, 30, 81,1.0)',
                borderColor: 'rgba(111, 30, 81,1.0)',
                borderWidth: 1
            }]
        },
        options: {
